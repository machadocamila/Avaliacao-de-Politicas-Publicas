...
for (year in c(1992, 1993, 1995)) {
  ...
  df_psm <- temp %>% 
    filter(ano == year) %>% 
    select(c(tratamento, idade, anos_estudo, peso_pessoa, ultima_serie_concluida, renda_dom_per_capita,
...
model <- feols(anos_estudo ~ tratamento + i(ano, tratamento, ref = "1992"),
               data = df_psm,
               vcov = "hetero")
summary(model)
